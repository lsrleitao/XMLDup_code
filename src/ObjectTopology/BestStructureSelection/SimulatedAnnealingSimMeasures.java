...
public class SimulatedAnnealingSimMeasures {
    ...
    private static String _configFile = "./XMLDup Data/DBConfigFile/config_arruamentos.xml";
    private static String _dbPath = "./XMLDup Data/DB/arruamentos.xml";
    private static int _maxNumberSAIterations = 0;

    //Percentagens dos varios tipos de pares a figurar na amostra
    private static int _criticalPairspercentageTP = 10;
    private static int _criticalPairspercentageFP = 10;
    private static int _criticalPairspercentageTN = 10;
    private static int _criticalPairspercentageFN = 10;
    
    private static String[] _simMeasures = {
    ...
    private static String[] initializeSM(String[] defaultSM, String simMeasure){
        for(int i = 0; defaultSM.length > i; i++){
            defaultSM[i] = simMeasure;
        }
    ...
    private static String[] onlineNeighbor(String[] state){

        int attributeSM_index = generateRandomNumberRange(0,state.length-1);
        
        int attributeSM = generateRandomNumberRange(0,_simMeasures.length-1);
        
        state[attributeSM_index] = _simMeasures[attributeSM];
        
        String key = generateKey(state);

        while(_visitedStructures.contains(key)){System.out.println("HIT!");
            attributeSM_index = generateRandomNumberRange(0,state.length-1);
            attributeSM = generateRandomNumberRange(0,_simMeasures.length-1);
            state[attributeSM_index] = _simMeasures[attributeSM];           
    ...
    private static Document loadPFToConfigFile(Document d, String[] state, List<String> attributes){
        ...
        for(int i = 0; attributes.size() > i; i++){
            ...
            e.setAttribute("simMeasure", state[i]);
        }
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            String[] smConfig = new String[attributes.size()];
            ...
            smConfig = initializeSM(smConfig,"levenstein");      

            //SA algorithm
            int chagesCnt = 0;
            String[] s = copyState(smConfig);
            ...
            while(i > 0){
                ...
                String[] s_new = copyState(onlineNeighbor(copyState(s)));  
                ...
                if(p_new > p_best){
                    ...
                    doc = (Document) document.cloneNode(true);
                    bestScoreStates.add(doc);
                    System.out.println("Novo Maximo: " + s_best[0]);
                }
